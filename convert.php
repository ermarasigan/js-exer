<?php
	...
	for($i=0; $i<sizeof($chords) ;$i++) {
		...
		for($j=0; $j<sizeof($chordtemp); $j++) {
			array_push($chordlist,$chordtemp[$j]);
		} 
	}
	...
	// 		];
			...
			foreach($chordlist as $chord) {
				if(isset($tabs[$chord])){
					...
					// size='L' 
					echo "<uke-chord frets='$tab'  position=$pos name='$chord'  style='background: white; padding-right: 20px; margin: 10px; border: 3px solid blue; border-radius: 20px'></uke-chord>";
				}
	}
?>
